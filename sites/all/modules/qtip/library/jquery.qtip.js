	...
	*/
	...
	function assignEvents()
	{
		...
		{ 
			...
			if(options.show.delay > 0) {
				...
				self.timers.show = setTimeout(callback, options.show.delay);
					...
					$('#genesis-1c').addClass('bgcl');
					$('#container').addClass('hidebg');
		...
		function hideMethod(event)
		{
			...
			if((posOptions.target === 'mouse' && ontoTooltip) || (options.hide.fixed && ((/mouse(leave|out|move)/).test(event.type) && (ontoTooltip || ontoTarget)))) {
		...
		tooltip.bind('mouseenter'+namespace+' mouseleave'+namespace, function(event) {
			...
			if(state) { self.focus(event); //alert('byee');
			   //  $('#genesis-1c').addClass('bgcl');
//  			    $('#container').addClass('hidebg');
			}
		...
		if(/mouse(out|leave)/i.test(options.hide.event)) {
		   ...
		   //  alert('hello');
		    showMethod('mousenter');
			    $('#genesis-1c').addClass('bgcl');
 			    $('#container').addClass('hidebg');
		       
			    if(options.hide.leave === 'window') {//alert(options.hide.leave);
